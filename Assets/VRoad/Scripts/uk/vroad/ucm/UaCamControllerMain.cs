...
namespace uk.vroad.ucm
{
    ...
    public abstract class UaCamControllerMain : MonoBehaviour, LAppState, LAppInput
    {
        ...
        {
            if (trackingGO == go) { trackingGO = null; isTracking = false; }
        }
...
#if UNITY_WEBGL
        public Toggle carCam;
        public bool IsAutoTracking()
        {
            bool autoWasOn = autoTrack;
            autoTrack = carCam.isOn;
            if (!autoTrack && autoWasOn) TrackThis(null);
            return autoTrack;
        }

        public virtual void GoToMapCentre() { carCam.isOn = false; } // autoTrack cleared on next call to IsAutoTracking
#else
        public bool IsAutoTracking() { return autoTrack; }
